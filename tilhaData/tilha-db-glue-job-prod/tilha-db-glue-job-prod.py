...
04/
12/
table.tilha_fts_event.1681334540847.batch.1681334540847.fullscanned.true.csv.gz
...
## @params: [JOB_NAME]
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'secretname', 'db_table', 'prod_trgt_bucket'])#dev bukettia ei tarvita, tilha ei ole ajossa devissä.
...
extract_bucket = 'tilha-ade-landing-bucket' # On this account
source_bucket = 'tilha-ade-landing-bucket' # On this account
#test_extract_bucket = 'aaa-tilhatestilandingbuketti' # On this account
#test_source_bucket = 'aaa-tilhatestilandingbuketti' # On this account
#
#
prefix = 'tilha/'  
#
...
def get_export_file_name(import_file_name):  # File: tilha-ade-landing-bucket/fts_event/part-00000-5a2813e0-9c57-4669-8d12-e197d472927a-c000.csv.gz
    ...
    #folder_name = "tilha_" + file_name_new
...
def fetch_filenames_s3(bucket, prefix):    # (source_bucket, prefix) = (tilha-ade-landing-bucket, 'tilha/')
    ...
    #print("IMPORT PATH: {}".format(prefix))  #tilha-ade-landing-bucket/fts_event/part-00000-5a2813e0-9c57-4669-8d12-e197d472927a-c000.csv.gz, ....
    #print(keys)
    return keys
...
def copy_files_prod(bucket, filenames):
    ...
    for file in filenames:
        ...
        try:
            print("PROCESSING: \"{}\"".format(str(file))) #tilha-ade-landing-bucket/fts_event/part-00000-5a2813e0-9c57-4669-8d12-e197d472927a-c000.csv.gz
...
def read_data_from_sourcedb():
    ...
    for table in tables.split(','):
        ...
        "user": username,
        "password": password
        #"customJdbcDriverS3Path": "s3://tilha-bucket/install/postgresql-42.3.7.jar",
        #"customJdbcDriverClassName": "postgresql.jdbc.OracleDriver"
        }
        ...
        df_sql=spark.sql(sql).repartition(40).write.format("csv").mode("overwrite").option("delimiter",";").option("quoteAll","True").option("escape",'"').option("header","False").option("codec","gzip").save("s3a://%s/tilha/%s" % (extract_bucket, folder))
