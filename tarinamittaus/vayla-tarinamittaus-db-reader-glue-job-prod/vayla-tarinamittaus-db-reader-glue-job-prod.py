...
## @params: [JOB_NAME]
print('Vaihe 1')
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'secretname', 'db_table', 'prod_trgt_bucket'])
print('Vaihe 2')
sc = SparkContext()
print('Vaihe 3')
...
extract_bucket = 'tarinamittaus-landing-bucket-prod' # On this account
source_bucket = 'tarinamittaus-landing-bucket-prod' #Ympäristönmuuttujiin?
...
def get_secret(secretname):
    ...
    return username, password, host, dbname, engine, port
...
def get_export_file_name(import_file_name):  # File: talous-sampo-data/vt_liikenne12_obs/part-00000-b3326108-86dd-4317-a2e4-c0c1440fbe80-c000.csv.gz
    ...
    """
    print('Generating an ADE compatible file name for: ' + import_file_name)
    ...
    file_prefix = "table.tarinamittaus_"
...
def fetch_filenames_s3(bucket, prefix):    # (source_bucket, prefix) = (sillaritest, 'talous-sampo-data/')
    print('Fetching filenames from s3 bucket: ' + bucket + '/' + prefix)
...
def copy_files_prod(bucket, filenames):
    ...
    for file in filenames:
        ...
        try:
            print("Processing file: \"{}\"".format(str(file))) #talous-sampo-data/vt_liikenne12_obs/part-00000-b3326108-86dd-4317-a2e4-c0c1440fbe80-c000.csv.gz
            ...
            print(f'Copying file: {file} \nFrom bucket: {copy_src} \nTo target bucket: {prod_target_bucket}/{prefix}')
            ...
            print('Successfully copied file: ' + file)
...
def read_data_from_sourcedb():
    print('Vaihe 8')
    username, password, host, dbname, engine, port = get_secret(args['secretname'])
    print('Vaihe 9')
    ...
    for table in tables.split(','):
        ...
        "url": f'jdbc:{engine}://{host}:{port}/{dbname}',
        "dbtable": table,
        "user": username,
        "password": password
        #"customJdbcDriverS3Path": "s3://talous-sampo-ade-landingbucket/oracle-driver/ojdbc8.jar",
        #"customJdbcDriverClassName": "oracle.jdbc.OracleDriver"
        }'''
                # Read DynamicFrame from Oracle
        #source_df = spark.read.format("jdbc").option("url",f'jdbc:postgresql:"{connection_postgresql_options[url]};User={username};Password={password};Database={dbname}').option("dbtable",table).option("driver","cdata.jdbc.postgresql.PostgreSQLDriver").load()
        #df_postgresql = glueContext.create_dynamic_frame.from_options(connection_type="postgresql", connection_options=connection_postgresql_options)
        #print('Vaihe 13')
        #print("rows = {}".format(df_oracle8.count()))  
        #df=df_postgresql.toDF()
        connection_url=f'jdbc:{engine}://{host}:{port}/{dbname}?user={username}&password={password}&sslfactory=org.postgresql.ssl.NonValidatingFactory'
        df = spark.read.format("jdbc").option("url", connection_url).option("dbtable", table).load()        
        cols=df.columns
        print('Vaihe 12, 13, 14')
        cols=df.columns
        print('Vaihe 15: kolumnit')
        print(cols)
        #print(cols)
        ...
        print('Vaihe 16: select')
        print('SQL:' + sql)
        df2=df.registerTempTable("data_table")
        print('Vaihe 17: This is only temporary')
        folder=table.split(".")[1]
        print('Vaihe 18')
        df_sql=spark.sql(sql).coalesce(1).write.format("csv").mode("overwrite").option("delimiter",";").option("quoteAll","True").option("escape",'"').option("header","True").option("codec","gzip").save("s3a://%s/tarinamittaus/%s" % (extract_bucket, folder))
        print('Vaihe 19: FUNKTIO PÄÄSI MAALIIN!')
    print("Source data copied from all tables!")
...
#source_df = spark.read.format("jdbc").option("url",f'jdbc:postgresql://{host}:{port}/{dbname};User={username};Password={password};Database={dbname}').option("dbtable","tarinam.mittaus").load()
